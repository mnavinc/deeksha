...
CREATE TYPE member_role AS ENUM ('GROUP_ADMIN', 'EXPENSE_MANAGER', 'MEMBER');
CREATE TYPE deeksha_status AS ENUM ('ACTIVE', 'PAUSED', 'COMPLETED', 'CANCELLED');
CREATE TYPE expense_split AS ENUM ('EQUAL', 'EXACT', 'PERCENTAGE', 'SHARES', 'SELECTED');
CREATE TYPE settlement_status AS ENUM ('PENDING', 'SETTLED', 'VOID');
CREATE TYPE verification_status AS ENUM ('UNVERIFIED', 'COMMUNITY_VERIFIED', 'PARTNER_VERIFIED', 'OFFICIALLY_VERIFIED');
...
CREATE INDEX temples_location_idx ON temples (state, deity);
CREATE TABLE deeksha_enrollments (
...
CREATE UNIQUE INDEX one_active_enrollment_per_user ON deeksha_enrollments(user_id) WHERE status IN ('ACTIVE','PAUSED');
...
CREATE TABLE pilgrimage_groups (
...
CREATE INDEX expenses_group_idx ON expenses(group_id, expense_date DESC) WHERE NOT is_deleted;
...
CREATE TRIGGER temples_touch BEFORE UPDATE ON temples FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
CREATE TRIGGER enrollment_touch BEFORE UPDATE ON deeksha_enrollments FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
CREATE TRIGGER daily_log_touch BEFORE UPDATE ON daily_logs FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
CREATE TRIGGER groups_touch BEFORE UPDATE ON pilgrimage_groups FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
...
CREATE POLICY own_user ON users USING (id = app_user_id()) WITH CHECK (id = app_user_id());
CREATE POLICY own_enrollment ON deeksha_enrollments USING (user_id = app_user_id()) WITH CHECK (user_id = app_user_id());
CREATE POLICY own_logs ON daily_logs USING (EXISTS (SELECT 1 FROM deeksha_enrollments e WHERE e.id = enrollment_id AND e.user_id = app_user_id())) WITH CHECK (EXISTS (SELECT 1 FROM deeksha_enrollments e WHERE e.id = enrollment_id AND e.user_id = app_user_id()));
CREATE POLICY group_visible ON pilgrimage_groups USING (created_by=app_user_id() OR EXISTS (SELECT 1 FROM group_members gm WHERE gm.group_id=id AND gm.user_id=app_user_id())) WITH CHECK (created_by=app_user_id());
CREATE POLICY group_member_visible ON group_members USING (EXISTS (SELECT 1 FROM pilgrimage_groups g WHERE g.id=group_id AND (g.created_by=app_user_id() OR EXISTS (SELECT 1 FROM group_members mine WHERE mine.group_id=g.id AND mine.user_id=app_user_id())))) WITH CHECK (user_id=app_user_id() AND EXISTS (SELECT 1 FROM pilgrimage_groups g WHERE g.id=group_id AND g.created_by=app_user_id()));
CREATE POLICY group_expense_visible ON expenses USING ((group_id IS NULL AND created_by=app_user_id()) OR EXISTS (SELECT 1 FROM group_members gm WHERE gm.group_id=expenses.group_id AND gm.user_id=app_user_id())) WITH CHECK ((group_id IS NULL AND created_by=app_user_id()) OR EXISTS (SELECT 1 FROM group_members gm WHERE gm.group_id=expenses.group_id AND gm.user_id=app_user_id()));
CREATE POLICY expense_participant_visible ON expense_participants USING (EXISTS (SELECT 1 FROM expenses e WHERE e.id=expense_id AND (e.created_by=app_user_id() OR EXISTS (SELECT 1 FROM group_members gm WHERE gm.group_id=e.group_id AND gm.user_id=app_user_id()))));
CREATE POLICY settlement_visible ON settlements USING (EXISTS (SELECT 1 FROM group_members gm WHERE gm.group_id=settlements.group_id AND gm.user_id=app_user_id()));
